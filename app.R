...
ui <- dashboardPage(
  dashboardHeader(title = "LESOTHO AHD REPORT"),
  dashboardSidebar(
    tags$head(
      tags$style(HTML("
        .sidebar-menu li a {
    ...
    sidebarMenu(
      id = "tabs",
      menuItem("Guideline", tabName = "guideline", icon = icon("info-circle")),
  ...
  dashboardBody(
    tabItems(
      tabItem(tabName = "guideline",
              fluidRow(
                box(title = "Guideline", status = "info", solidHeader = TRUE, width = 12, class = "guideline-box",
                    "This dashboard is for visualizing the AHD report from Lesotho. Select the information from the tabs to visualize the report."
                )
              )
      ),
      tabItem(tabName = "screening_enrollment",
              fluidRow(
                ...
                box(title = "Eligible for AHD Screening (pre and post)", status = "primary", solidHeader = TRUE, width = 12, class = "box-title",
                    tableOutput("table2")),
                box(title = "Eligible for AHD Screening", status = "primary", solidHeader = TRUE, width = 12, class = "box-title",
                    tableOutput("table3")),
                box(title = "Total number diagnosed with AHD", status = "primary", solidHeader = TRUE, width = 12, class = "box-title",
                    tableOutput("table4")),
                box(title = "Diagnosed with AHD in pre and post", status = "primary", solidHeader = TRUE, width = 12, class = "box-title",
                    tableOutput("table5")),
                box(title = "Not eligible for prospective cohort", status = "primary", solidHeader = TRUE, width = 12, class = "box-title",
                    tableOutput("table6")),
                box(title = "Eligible for consent", status = "primary", solidHeader = TRUE, width = 12, class = "box-title",
                    tableOutput("table7")),
                box(title = "Prospective clients", status = "primary", solidHeader = TRUE, width = 12, class = "box-title",
                    tableOutput("table8")),
                box(title = "Total entries", status = "primary", solidHeader = TRUE, width = 12, class = "box-title",
                    tableOutput("table9"))
              )
      ),
...
server <- function(input, output, session) {
  ...
  output$table1 <- renderTable({
    clean_table(Total_AHD_screening)
  })
  
  output$table2 <- renderTable({
    clean_table(Eligible_AHD_pre_post)
  })
  
  output$table3 <- renderTable({
    clean_table(Eligible_AHD)
  })
  
  output$table4 <- renderTable({
    clean_table(Total_diag_AHD)
  })
  
  output$table5 <- renderTable({
    clean_table(Diag_AHD_pre_post)
  })
  
  output$table6 <- renderTable({
    clean_table(Not_elig_pros_coh)
  })
  
  output$table7 <- renderTable({
    clean_table(Elig_consent)
  })
  
  output$table8 <- renderTable({
    clean_table(Prospective_clients)
  })
  
  output$table9 <- renderTable({
    clean_table(Total_entries)
  })
  ...
  output$table11 <- renderTable({
    clean_table(Sum_Enl_Pre)
  })
  
  output$table12 <- renderTable({
    clean_table(Sum_Enl_Post)
  })
 
  # Followup in the prospective cohort
  output$table13 <- renderTable({
    clean_table(Eligible_for_consent)
  })
  
  output$table14 <- renderTable({
    clean_table(Interviewed_Enr)
  })
  
  output$table15 <- renderTable({
    clean_table(Expected_at_3)
  })
  
  output$table16 <- renderTable({
    clean_table(Interviewed_at_3)
  })
  
  output$table17 <- renderTable({
    clean_table(Expected_at_6)
  })
  
  output$table18 <- renderTable({
    clean_table(Interviewed_at_6)
  })
